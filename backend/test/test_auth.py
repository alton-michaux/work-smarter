...
def test_login_success(api_client, create_user):
    ...
    response = api_client.post("/api/auth/login/", {"username": user.username, "password": "madhatter"}, format="json")
    assert response.status_code == 200
    assert "key" in response.data or "access" in response.data

@pytest.mark.django_db
def test_login_fail(api_client):
    response = api_client.post("/api/auth/login/", {"username": "fake", "password": f"wrong_password"}, format="json")
...
def test_logout(api_client, create_user):
    ...
    login_response = api_client.post("/api/auth/login/", {"username": user.username, "password": "canwefixit"}, format="json")
